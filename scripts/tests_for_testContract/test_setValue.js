...
const callData = contract.interface.encodeFunctionData("updateMapto0WithRequire");
...
const outputPath = path.join(__dirname, "updateMapto0WithRequire-trace-format1.txt");
fs.writeFileSync(
    outputPath,
    opcodeSequence.join("\n")
...
const opcodeSequenceWithArgs = trace.structLogs.map((log) => {
    ...
    const opcodeArgsOutputPath = path.join(
        __dirname,
        "updateMapto0WithRequire-trace-format2.txt"
    );
...
const tracePath = path.join(
    __dirname,
    "updateMapto0WithRequire-trace-format3.json"
);
