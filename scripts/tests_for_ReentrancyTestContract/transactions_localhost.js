...
// ************************** function ****************************** //
const callData = contract.interface.encodeFunctionData("callTransferUseAmount");
...
const tx = await signers[0].sendTransaction({
    ...
    data: callData,
     nonce: nonce,
    gasLimit: 300000, // Set a higher gas limit to avoid out-of-gas errors
...
const outputPath = path.join(__dirname, "callTransferUseAmount-format1.txt");
fs.writeFileSync(
    outputPath,
    opcodeSequence.join("\n")
);
...
const opcodeSequenceWithArgs = trace.structLogs.map((log) => {
    ...
    const opcodeArgsOutputPath = path.join(
        __dirname,
        "callTransferUseAmount-format2.txt"
    );
...
const tracePath = path.join(
    __dirname,
    "callTransferUseAmount-format3.json"
);
